...
// ---- SHA-512 (2-way multi-buffer; 64-bit lanes) ----

#if defined(__SSE2__)
typedef __m128i v2;
#define V2_ADD(a, b) _mm_add_epi64((a), (b))
#define V2_AND(a, b) _mm_and_si128((a), (b))
#define V2_OR(a, b) _mm_or_si128((a), (b))
#define V2_XOR(a, b) _mm_xor_si128((a), (b))
#define V2_ANDNOT(a, b) _mm_andnot_si128((a), (b)) /* ~a & b */
#define V2_SHR(a, n) _mm_srli_epi64((a), (n))
#define V2_SHL(a, n) _mm_slli_epi64((a), (n))
#define V2_SET1(x) _mm_set1_epi64x((long long)(x))
static inline v2 v2_set(uint64_t w0, uint64_t w1) {
  return _mm_set_epi64x((long long)w1, (long long)w0); /* lane0 = w0 */
}
static inline void v2_get(v2 v, uint64_t o[2]) {
  _mm_storeu_si128((__m128i *)o, v);
}
#elif defined(__aarch64__) || defined(__ARM_NEON)
typedef uint64x2_t v2;
#define V2_ADD(a, b) vaddq_u64((a), (b))
#define V2_AND(a, b) vandq_u64((a), (b))
#define V2_OR(a, b) vorrq_u64((a), (b))
#define V2_XOR(a, b) veorq_u64((a), (b))
#define V2_ANDNOT(a, b) vbicq_u64((b), (a)) /* b & ~a */
#define V2_SHR(a, n) vshrq_n_u64((a), (n))
#define V2_SHL(a, n) vshlq_n_u64((a), (n))
#define V2_SET1(x) vdupq_n_u64((uint64_t)(x))
static inline v2 v2_set(uint64_t w0, uint64_t w1) {
  uint64_t t[2] = {w0, w1};
  return vld1q_u64(t);
}
static inline void v2_get(v2 v, uint64_t o[2]) { vst1q_u64(o, v); }
#else
typedef struct {
  uint64_t l[2];
} v2;
static inline v2 V2_ADD(v2 a, v2 b) {
  v2 r;
  for (int i = 0; i < 2; i++) r.l[i] = a.l[i] + b.l[i];
  return r;
}
static inline v2 V2_AND(v2 a, v2 b) {
  v2 r;
  for (int i = 0; i < 2; i++) r.l[i] = a.l[i] & b.l[i];
  return r;
}
static inline v2 V2_OR(v2 a, v2 b) {
  v2 r;
  for (int i = 0; i < 2; i++) r.l[i] = a.l[i] | b.l[i];
  return r;
}
static inline v2 V2_XOR(v2 a, v2 b) {
  v2 r;
  for (int i = 0; i < 2; i++) r.l[i] = a.l[i] ^ b.l[i];
  return r;
}
static inline v2 V2_ANDNOT(v2 a, v2 b) {
  v2 r;
  for (int i = 0; i < 2; i++) r.l[i] = (~a.l[i]) & b.l[i];
  return r;
}
static inline v2 V2_SHR(v2 a, int n) {
  v2 r;
  for (int i = 0; i < 2; i++) r.l[i] = a.l[i] >> n;
  return r;
}
static inline v2 V2_SHL(v2 a, int n) {
  v2 r;
  for (int i = 0; i < 2; i++) r.l[i] = a.l[i] << n;
  return r;
}
static inline v2 V2_SET1(uint64_t x) {
  v2 r;
  r.l[0] = x;
  r.l[1] = x;
  return r;
...
#define R64(x, n) V2_OR(V2_SHR((x), (n)), V2_SHL((x), (64 - (n))))
#define s0_512(x) V2_XOR(V2_XOR(R64(x, 1), R64(x, 8)), V2_SHR(x, 7))
#define s1_512(x) V2_XOR(V2_XOR(R64(x, 19), R64(x, 61)), V2_SHR(x, 6))
#define b0_512(x) V2_XOR(V2_XOR(R64(x, 28), R64(x, 34)), R64(x, 39))
#define b1_512(x) V2_XOR(V2_XOR(R64(x, 14), R64(x, 18)), R64(x, 41))
...
// SHA-512 padding: 0x80 + zeros + 16-byte big-endian length, to a multiple of
// 128. Two messages.
static uint8_t *pad2_512(const uint8_t *m0, const uint8_t *m1, int n,
                         size_t *plen_out, int *blocks_out) {
  size_t plen = ((size_t)(n + 16) / 128) * 128 + 128;
  uint8_t *pad = (uint8_t *)calloc(2, plen);
  const uint8_t *ms[2] = {m0, m1};
  uint64_t bits = (uint64_t)n * 8;
  for (int k = 0; k < 2; k++) {
    uint8_t *p = pad + (size_t)k * plen;
    if (n > 0) memcpy(p, ms[k], (size_t)n);
  ...
  *plen_out = plen;
  *blocks_out = (int)(plen / 128);
  return pad;
}

void mb_sha512_x2(const uint8_t *m0, const uint8_t *m1, int n, uint8_t *out) {
  size_t plen;
  int blocks;
  uint8_t *pad = pad2_512(m0, m1, n, &plen, &blocks);
  v2 H[8];
  for (int i = 0; i < 8; i++) H[i] = V2_SET1(IV512[i]);
  v2 W[80];
  for (int blk = 0; blk < blocks; blk++) {
    for (int t = 0; t < 16; t++) {
      uint64_t w0 = rd_be64(pad + (size_t)blk * 128 + t * 8);
      uint64_t w1 = rd_be64(pad + plen + (size_t)blk * 128 + t * 8);
      W[t] = v2_set(w0, w1);
    }
    for (int t = 16; t < 80; t++)
      W[t] = V2_ADD(V2_ADD(s1_512(W[t - 2]), W[t - 7]),
                    V2_ADD(s0_512(W[t - 15]), W[t - 16]));
    v2 a = H[0], b = H[1], c = H[2], d = H[3], e = H[4], f = H[5], g = H[6],
       h = H[7];
    for (int t = 0; t < 80; t++) {
      v2 ch = V2_XOR(V2_AND(e, f), V2_ANDNOT(e, g));
      v2 t1 = V2_ADD(V2_ADD(V2_ADD(h, b1_512(e)), V2_ADD(ch, V2_SET1(K512[t]))),
                     W[t]);
      v2 mj = V2_XOR(V2_XOR(V2_AND(a, b), V2_AND(a, c)), V2_AND(b, c));
      v2 t2 = V2_ADD(b0_512(a), mj);
      h = g;
      g = f;
      f = e;
      e = V2_ADD(d, t1);
  ...
  for (int i = 0; i < 8; i++) {
    ...
    v2_get(H[i], o);
    for (int k = 0; k < 2; k++) {
      uint8_t *p = out + k * 64 + i * 8;
      for (int j = 0; j < 8; j++) p[j] = (uint8_t)(o[k] >> ((7 - j) * 8));
    }
  }
  free(pad);
}
